...
from cmu.gcl_t import Feature_learning_layer
import numpy as np
...
class SHAF(nn.Module):
    def __init__(self, in_node_nf, in_edge_nf, hidden_nf, in_channel, hid_channel, out_channel, device='cpu', act_fn=nn.SiLU(), n_layers=4, coords_weight=1.0, recurrent=False, norm_diff=False, tanh=False,add_agent_token=False,n_agent=25,category_num=4):
        ...
        self.embedding = nn.Linear(35, int(self.hidden_nf/2))
        self.embedding_v = nn.Linear(35, int(self.hidden_nf/2))
        self.embedding_a = nn.Linear(35, int(self.hidden_nf / 2))  
        self.coord_trans = nn.Linear(35, int(hid_channel), bias=False)
        self.vel_trans = nn.Linear(35, int(hid_channel), bias=False)
        self.predict_head = nn.Linear(hid_channel, 35, bias=False)
        self.apply_dct = True
        ...
        if self.add_agent_token:
             self.agent_embed = nn.Parameter(torch.randn(1, 12,96))
        ...
        self.embedding_1 = nn.Linear(35, int(self.hidden_nf / 2))  
        self.embedding_v_1 = nn.Linear(35, int(self.hidden_nf / 2))  
        self.embedding_a_1 = nn.Linear(35, int(self.hidden_nf / 2))  
        self.coord_trans_1 = nn.Linear(35, int(hid_channel), bias=False)  
        self.vel_trans_1 = nn.Linear(35, int(hid_channel), bias=False)  
        self.predict_head_1 = nn.Linear(hid_channel, 35, bias=False)  
        ...
        self.embedding_2 = nn.Linear(35, int(self.hidden_nf / 2))  
        self.embedding_v_2 = nn.Linear(35, int(self.hidden_nf / 2))  
        self.embedding_a_2 = nn.Linear(35, int(self.hidden_nf / 2))  
        self.coord_trans_2 = nn.Linear(35, int(hid_channel), bias=False)  
        self.vel_trans_2 = nn.Linear(35, int(hid_channel), bias=False)  
        self.predict_head_2 = nn.Linear(hid_channel, 35, bias=False)  
        ...
        self.A = nn.Parameter(torch.randn(3,25,25))
        ...
        self.mid_trans = nn.Sequential(
            ...
            nn.Linear(12, hidden_nf),  
            act_fn,
            nn.Linear(hidden_nf, 25),  
            act_fn)
    ...
    def forward(self, h, x,x_end, vel, edge_attr=None):
        ...
        if self.apply_dct:
            ...
            x_0 = x_0 - x_center_0 
            dct_m,_ = self.get_dct_matrix(35,x_0) 
            _,idct_m = self.get_dct_matrix(35,x_0)
        ...
        x_1=torch.cat([coord,x_mid[:,:,10:35]], dim=-2)
        ...
        if self.apply_dct:
            ...
            x_1 = x_1 - x_center_1 
            dct_m,_ = self.get_dct_matrix(35,x_1) 
            _,idct_m = self.get_dct_matrix(35,x_1)
            dct_m = dct_m[None,None,:,:].repeat(batch_size,agent_num,1,1)
            idct_m = idct_m[None,None,:,:].repeat(batch_size,agent_num,1,1)
        ...
        x_2=torch.cat([coord,x_1[:,:,10:35]], dim=-2)
        ...
        if self.apply_dct:
            ...
            x_2 = x_2 - x_center_2 
            dct_m,_ = self.get_dct_matrix(35,x_2) 
            _,idct_m = self.get_dct_matrix(35,x_2)
            dct_m = dct_m[None,None,:,:].repeat(batch_size,agent_num,1,1)
            idct_m = idct_m[None,None,:,:].repeat(batch_size,agent_num,1,1)
