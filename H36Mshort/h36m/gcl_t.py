...
class Feature_learning_layer(nn.Module):
    def __init__(self, input_nf, output_nf, hidden_nf, input_c, hidden_c, output_c, edges_in_d=0, nodes_att_dim=0, act_fn=nn.ReLU(), recurrent=True, coords_weight=1.0, attention=False, norm_diff=False, tanh=False,apply_reasoning=True, output_reasoning=False, input_reasoning=False,category_num=2):
        ...
        self.cat_w=nn.Parameter(torch.ones(1))
        ...
        self.coord_mlp2 = nn.Sequential(
            nn.Linear(72, hidden_nf),  
            act_fn,
            nn.Linear(hidden_nf, 72),  
            act_fn)
        ...
        self.tao=1
        self.category_mlp2 = nn.Sequential(
            nn.Linear(hidden_nf * 2 + 72, hidden_nf),  
            act_fn,
    ...
    def edge_model(self, h, coord, edge_attr=None):
        ...
        coord_dist = self.coord_mlp2(coord_dist)  
        edge_feat = torch.cat([h1,h2,coord_dist],dim=-1)
        edge_feat = self.edge_mlp(edge_feat)


        return edge_feat, coord_diff,coord_dist 

    def SFE(self, coord, edge_feat, coord_diff, category,h):
    ...
    def forward(self, h, coord, vel, edge_attr=None, node_attr=None,category=None):
        ...
        edge_feat, coord_diff,coord_dist = self.edge_model(h,coord,edge_attr)

        if self.add_TFE:
            coord = self.TFE(coord,h) 
        coord = self.SFE(coord, edge_feat, coord_diff, category,h)
        ...
        h = self.FU(h, edge_feat)

        node_new1 = h[:, :, None, :].repeat(1, 1, h.shape[1], 1)  
        node_new2 = h[:, None, :, :].repeat(1, h.shape[1], 1, 1)  
        edge_feat_input_new = torch.cat([node_new1, node_new2, coord_dist], dim=-1)  
        category_new = F.softmax(self.category_mlp2(edge_feat_input_new) / self.tao,dim=-1)  

        category=category+self.cat_w*category_new
