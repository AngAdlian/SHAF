...
import os
from TDPW_model import forward_kinematics
...
def rotmat2euler(R):
    ...
    https://github.com/asheshjain399/RNNexp/blob/srnn/structural_rnn/CRFProblems/H3.6m/mhmublv/Motion/RotMat2Euler.m
    Args
...
def rotmat2quat(R):
    ...
    Matlab port to python for evaluation purposes
    https://github.com/asheshjain399/RNNexp/blob/srnn/structural_rnn/CRFProblems/H3.6m/mhmublv/Motion/rotmat2quat.m
    Args
...
def quat2expmap(q):
    ...
    Matlab port to python for evaluation purposes
    https://github.com/asheshjain399/RNNexp/blob/srnn/structural_rnn/CRFProblems/H3.6m/mhmublv/Motion/quat2expmap.m
    Args
      q: 1x4 quaternion
    Returns
      r: 1x3 exponential map
...
def unNormalizeData(normalizedData, data_mean, data_std, dimensions_to_ignore, actions, one_hot):
    ...
    https://github.com/asheshjain399/RNNexp/blob/srnn/structural_rnn/CRFProblems/H3.6m/generateMotionData.py
    Args
...
def readCSVasFloat(filename):
    ...
    https://github.com/asheshjain399/NeuralModels/blob/master/neuralmodels/utils.py
    Args
...
def normalization_stats(completeData):
    ...
    https://github.com/asheshjain399/RNNexp/blob/srnn/structural_rnn/CRFProblems/H3.6m/processdata.py
    Args
...
def load_data_cmu_3d_all(path_to_dataset, actions, input_n, output_n, is_test=False):
...
def load_data_cmu_3d_n(path_to_dataset, actions, input_n, output_n, is_test=False):
    test_sample_num = -1
    seq_len = input_n + output_n
    nactions = len(actions)
    sampled_seq = []
...
def expmap2rotmat_torch(r):
    """
    Converts expmap matrix to rotation 将expmap矩阵转换为旋转
...
def expmap2xyz_torch(expmap):
    """
    convert expmaps to joint locations 将expmaps转换为关节位置
    :param expmap: N*99
    :return: N*32*3
    """
...
def load_data(path_to_dataset, subjects, actions, sample_rate, seq_len, input_n=10, data_mean=None, data_std=None):
    """
    adapted from
    https://github.com/una-dinosauria/human-motion-prediction/src/data_utils.py
        ...
        for action_idx in np.arange(len(actions)):
            ...
            if not (subj == 12):
                ...
                for subact in [1, 2]:  
                    ...
                    [2      3   4...    19   20   21]
                    ...
                    [1716   1717    1718... 1733    1734    1735]
                    [1717 1718 1719... 1734 1735 1736]
                    [1718   1719    1720...1735 1736    1737]]
                    """
                    seq_sel = the_sequence[fs_sel, :] 
                    if len(sampled_seq) == 0:
                        sampled_seq = seq_sel 
...
def find_indices_srnn(frame_num1, frame_num2, seq_len, input_n=10):
    """
    Adapted from https://github.com/una-dinosauria/human-motion-prediction/blob/master/src/seq2seq_model.py
...
def load_data_ntu(path_to_dataset,path_to_pkl,input_n, output_n):
    ...
    data = np.load(path_to_dataset, mmap_mode='r')
    
    
    
    for num in np.arange(data.shape[0]):
        if label[num]>8:
            continue
        lenth=seq_len[num]
        seq=data[num]
        seq=seq.transpose(3,1,2,0)
        seq = seq.transpose(1,0,2,3)
        seq = seq.reshape(seq.shape[0],-1,seq.shape[3])
        seq=seq[0:lenth]
        


        fs = np.arange(0, lenth - input_n - output_n + 1)  
        fs_sel = fs
        for i in np.arange(input_n + output_n - 1):
            fs_sel = np.vstack((fs_sel, fs + i + 1))  
        fs_sel = fs_sel.transpose()  
        seq_sel = seq[fs_sel, :]
        

        seq_sel=seq_sel.reshape(seq_sel.shape[0],seq_sel.shape[1],-1)
        ...
        if len(sampled_seq) == 0:
            ...
            sampled_seq = np.concatenate((sampled_seq, seq_sel), axis=0)  
            complete_seq = np.append(complete_seq, seq, axis=0)



    joint_to_ignore = np.array([20, 21, 22, 23, 24, 45, 46, 47, 48, 49])
    dimensions_to_ignore = np.concatenate((joint_to_ignore * 3, joint_to_ignore * 3 + 1,joint_to_ignore * 3 + 2))  
    dimensions_to_use = np.setdiff1d(np.arange(sampled_seq.shape[2]), dimensions_to_ignore)


    return sampled_seq, joint_to_ignore, dimensions_to_use
