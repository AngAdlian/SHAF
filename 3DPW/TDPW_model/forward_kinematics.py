...
from TDPW_model import data_utils
...
def _some_variables():
    ...
    Args
      None
    Returns
      parent: 32-long vector with parent-child relationships in the kinematic tree 运动树中具有父子关系的32个长向量
      offset: 96-long vector with bone lenghts  具有骨骼长度的96长矢量
      rotInd: 32-long list with indices into angles 32个带角度索引的长列表
      expmapInd: 32-long list with indices into expmap angles 32长列表，索引为expmap角度
    """
...
def fkl_torch(angles, parent, offset, rotInd, expmapInd):
    ...
    convert joint angles to joint locations  将关节角度转换为关节位置
    batch pytorch version of the fkl() method above
    :param angles: N*99  human3.6m中每一行有3*33个旋转向量 第一个为1738*99
    :param parent:
    :param offset:
    :param rotInd:
    :param expmapInd:
    :return: N*joint_n*3 第一个为1738*32*3 为关节坐标
    """
