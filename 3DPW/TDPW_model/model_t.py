...
from TDPW_model.gcl_t import Feature_learning_layer
import numpy as np
...
class AveargeJoint2(nn.Module):
    ...
    def __init__(self):
        ...
        self.joint2 = [5, 6, 7]
        self.joint3 = [8,9,10]
        self.joint4 = [11,12,13,14,15,16]
        self.joint5 = [17,18,19,20,21,22]

    def forward(self, x):
        x_joint0 = F.avg_pool2d(x[:, :, :, self.joint0], kernel_size=(1, 2))  
        x_joint1 = F.avg_pool2d(x[:, :, :, self.joint1], kernel_size=(1, 2))  
        x_joint2 = F.avg_pool2d(x[:, :, :, self.joint2], kernel_size=(1, 2))  
        x_joint3 = F.avg_pool2d(x[:, :, :, self.joint3], kernel_size=(1, 2))  
        x_joint4 = F.avg_pool2d(x[:, :, :, self.joint4], kernel_size=(1, 2))  
        x_joint5 = F.avg_pool2d(x[:, :, :, self.joint5], kernel_size=(1, 2))  
        x_body = torch.cat((x_joint0,x_joint1,x_joint2,x_joint3,x_joint4,x_joint5),
                           dim=-1)  
        return x_body 
...
class SHAF(nn.Module):
    def __init__(self, in_node_nf, in_edge_nf, hidden_nf, in_channel, hid_channel, out_channel, device='cpu', act_fn=nn.SiLU(), n_layers=4, coords_weight=1.0, recurrent=False, norm_diff=False, tanh=False,add_agent_token=False,n_agent=23,category_num=4):
        ...
        self.embedding = nn.Linear(40, int(self.hidden_nf/2))
        self.embedding_v = nn.Linear(40, int(self.hidden_nf/2))
        self.embedding_a = nn.Linear(40, int(self.hidden_nf / 2))  
        self.coord_trans = nn.Linear(40, int(hid_channel), bias=False)
        self.vel_trans = nn.Linear(40, int(hid_channel), bias=False)
        self.predict_head = nn.Linear(hid_channel, 40, bias=False)
        self.apply_dct = True
        ...
        if self.add_agent_token:
             self.agent_embed = nn.Parameter(torch.randn(1, 10,144))
        ...
        self.embedding_1 = nn.Linear(40, int(self.hidden_nf / 2))  
        self.embedding_v_1 = nn.Linear(40, int(self.hidden_nf / 2))  
        self.embedding_a_1 = nn.Linear(40, int(self.hidden_nf / 2))  
        self.coord_trans_1 = nn.Linear(40, int(hid_channel), bias=False)  
        self.vel_trans_1 = nn.Linear(40, int(hid_channel), bias=False)  
        self.predict_head_1 = nn.Linear(hid_channel, 40, bias=False)  

        self.tao_1 = 1

        if self.add_agent_token:
            self.agent_embed_1 = nn.Parameter(torch.randn(1, n_agent, 144))  
        ...
        self.embedding_2 = nn.Linear(40, int(self.hidden_nf / 2))  
        self.embedding_v_2 = nn.Linear(40, int(self.hidden_nf / 2))  
        self.embedding_a_2 = nn.Linear(40, int(self.hidden_nf / 2))  
        self.coord_trans_2 = nn.Linear(40, int(hid_channel), bias=False)  
        self.vel_trans_2 = nn.Linear(40, int(hid_channel), bias=False)  
        self.predict_head_2 = nn.Linear(hid_channel, 40, bias=False)  

        self.tao_1 = 1

        if self.add_agent_token:
            self.agent_embed_2 = nn.Parameter(torch.randn(1, n_agent, 144))  
        ...
        self.A = nn.Parameter(torch.randn(3,23,23))
    ...
    def forward(self, h, x,x_end, vel, edge_attr=None):
        ...
        if self.apply_dct:
            ...
            x_0 = x_0 - x_center_0 
            dct_m,_ = self.get_dct_matrix(40,x_0) 
            _,idct_m = self.get_dct_matrix(40,x_0)
        ...
        x_1=torch.cat([coord,x_mid[:,:,10:40]], dim=-2)
        ...
        if self.apply_dct:
            ...
            x_1 = x_1 - x_center_1 
            dct_m,_ = self.get_dct_matrix(40,x_1) 
            _,idct_m = self.get_dct_matrix(40,x_1)
            dct_m = dct_m[None,None,:,:].repeat(batch_size,agent_num,1,1)
            idct_m = idct_m[None,None,:,:].repeat(batch_size,agent_num,1,1)
        ...
        x_2=torch.cat([coord,x_1[:,:,10:40]], dim=-2)
        ...
        if self.apply_dct:
            ...
            x_2 = x_2 - x_center_2 
            dct_m,_ = self.get_dct_matrix(40,x_2) 
            _,idct_m = self.get_dct_matrix(40,x_2)
            dct_m = dct_m[None,None,:,:].repeat(batch_size,agent_num,1,1)
            idct_m = idct_m[None,None,:,:].repeat(batch_size,agent_num,1,1)
