import argparse
import torch
from TDPW_model.dataloader import DPW_Datasets
from TDPW_model.model_t import SHAF
import os
...
parser.add_argument('--lr', type=float, default=5e-4, metavar='N',
                    help='learning rate')
parser.add_argument('--epoch_decay', type=int, default=10, metavar='N',
...
parser.add_argument('--nf', type=int, default=96, metavar='N',
                    help='learning rate')
...
parser.add_argument('--tanh', type=eval, default=False, metavar='N',
                    help='use tanh')
parser.add_argument('--model_save_dir', type=str, default='TDPW_model/saved_models',
...
def main():
    ...
    if args.debug:
        dataset_train = DPW_Datasets(actions='walking', input_n=args.past_length, output_n=args.future_length, split=0, scale=args.scale)
    else:
        dataset_train = DPW_Datasets(actions='all', input_n=args.past_length, output_n=args.future_length, split=0, scale=args.scale)
    ...
    loaders_test = {}
    for act in acts:
        dataset_test = DPW_Datasets(actions=act, input_n=args.past_length, output_n=args.future_length, split=2, scale=args.scale)
        loaders_test[act] = torch.utils.data.DataLoader(dataset_test, batch_size=args.batch_size, shuffle=False, drop_last=False,num_workers=0)#源代码初始为num_workers=8   #测试数据
    ...
    if args.test:
        ...
        elif args.future_length == 30:
            avg_mpjpe = np.zeros((5))
        else:
    ...
    for epoch in range(0, args.epochs):#args.epochs=80
        if args.apply_decay:
...
class AveargeJoint2(nn.Module):
    ...
    def forward(self, x):
        x_joint0 = F.avg_pool2d(x[:, :, :, self.joint0], kernel_size=(1, 2))  # [N, C, T, V=1]
        x_joint1 = F.avg_pool2d(x[:, :, :, self.joint1], kernel_size=(1, 2))  # [N, C, T, V=1]
        x_joint2 = F.avg_pool2d(x[:, :, :, self.joint2], kernel_size=(1, 2))  # [N, C, T, V=1]
        x_joint3 = F.avg_pool2d(x[:, :, :, self.joint3], kernel_size=(1, 2))  # [N, C, T, V=1]
        x_joint4 = F.avg_pool2d(x[:, :, :, self.joint4], kernel_size=(1, 2))  # [N, C, T, V=1]
        x_joint5 = F.avg_pool2d(x[:, :, :, self.joint5], kernel_size=(1, 2))  # [N, C, T, V=1]
...
def train(model, optimizer, epoch, loader, dim_used=[], backprop=True):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        loc, vel, loc_end, loc_end_ori, item = data #loc为(100,22, 10, 3)22个关节10帧三维坐标，vel为100组22个关节10帧两针间坐标差，loc_end100组22个关节10到20帧三维坐标

        loc = loc.to(torch.float32)
        vel = vel.to(torch.float32)
        loc_end = loc_end.to(torch.float32)
        loc_end_ori = loc_end_ori.to(torch.float32)
        loc_start = loc[:,:,-1:]
        ...
        if args.weighted_loss:
            ...
            loss_10_pre = torch.mean(torch.norm(loc_pred_2[:, :, 10:40] - loc_end[:, :, 10:40], dim=-1))
    ...
    #                   float(t_3d.mean()) / N))
    # elif args.future_length == 30:
    #     print('Act: {},  ErrT: {:.3f} {:.3f} {:.3f} {:.3f} {:.3f}, TestError {:.4f}' \
    #           .format(actname,
    #                   float(t_3d[0]) / N, float(t_3d[1]) / N, float(t_3d[2]) / N, float(t_3d[3]) / N,
    #                   float(t_3d[4]) / N, float(t_3d.mean()) / N))
    # else:
...
def test(model, optimizer, epoch, act_loader,dim_used=[],backprop=False):
    ...
    if args.future_length == 60:
        ...
        print('Act: {},  ErrT: {:.3f} {:.3f} {:.3f} {:.3f} {:.3f}, TestError {:.4f}' \
              .format(actname,float(t_3d[0]) / N, float(t_3d[1]) / N, float(t_3d[2]) / N, float(t_3d[3]) / N,float(t_3d[4]) / N, float(t_3d.mean()) / N))
    else:
        print('Act: {},  ErrT: {:.3f} {:.3f} {:.3f} {:.3f}, TestError {:.4f}'\
            .format(actname, 
                    float(t_3d[0])/N, float(t_3d[1])/N, float(t_3d[2])/N, float(t_3d[3])/N, 
